...
SEGMENT_Error_t SEGMENT_DISPLAY(unsigned char digit, unsigned char display) {
    if (display == 1) {
        PORTD |= (1 << Display1Pin); // Enable Display1
        PORTD &= ~(1 << Display2Pin); // Disable Display2
    } else {
        PORTD &= ~(1 << Display1Pin); // Disable Display1
        PORTD |= (1 << Display2Pin); // Enable Display2
    }
    SegmentsPort = SevenSeg[digit];
}
